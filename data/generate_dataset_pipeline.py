...
from data.generate_data import import_openml_data_no_transform
...
def data_to_train_test(x, y, config, rng=None):
    ...
    #     y_test = y_test[:config["max_test_samples"]]
        if config["max_train_samples"]:
            ...
            train_set_prop = config["train_prop"]
        if config["train_prop"] > train_set_prop:
            x, _, y, _ = train_test_split(x, y, train_size=train_set_prop, random_state=rng)
        x_train, x_val_test, y_train, y_val_test = train_test_split(x, y, train_size=config["train_prop"], random_state=rng)
        x_val, x_test, y_val, y_test = train_test_split(x_val_test, y_val_test, train_size= config["val_test_prop"], random_state=rng)
    if config["max_val_samples"] and x_val.shape[0] > config["max_val_samples"]:
        x_val = x_val[:config["max_val_samples"]]
        y_val = y_val[:config["max_val_samples"]]
    if config["max_test_samples"] and x_test.shape[0] > config["max_test_samples"]:
        x_test = x_test[:config["max_test_samples"]]
        y_test = y_test[:config["max_test_samples"]]
    return x_train, x_val, x_test, y_train, y_val, y_test
