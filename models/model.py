...
class BiSHop(torch.nn.Module):
  ...
  device       : Model device.
  """
  def __init__(self, n_cat, n_num, n_out, n_class=5000, emb_dim=32, out_dim=24, patch_dim=8, factor=8, flip=True, n_agg=4, 
               actv='entmax', hopfield=True, d_model=512, d_ff=None, n_heads=8, e_layer=3, d_layer=3, 
               dropout=0.2, share=True, share_div=8, share_add=False, full_dropout=False, emb_dropout=0.1,
               mlp_actv=torch.nn.ReLU(), mlp_bn=True, mlp_bn_final=False, mlp_dropout=0.2,
               mlp_hidden=(4,2,1), mlp_skip=True, mlp_softmax=False, device=torch.device('cuda:0')):
